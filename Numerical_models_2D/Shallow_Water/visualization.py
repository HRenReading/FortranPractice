...
with open("results/initial", "rb") as f:
    h0 = np.fromfile("results/initial", dtype=np.float32).reshape((nx, ny), order='F')

plt.figure(figsize=(8, 5))
plt.imshow(h0, origin="lower", extent=[0, Lx, 0, Ly],
...
for fname in files:
    with open(os.path.join(result_dir, fname), 'rb') as f:
        h = np.fromfile(f, dtype=np.float32).reshape((nx, ny), order="F")
        h_series.append(h)

# --- Set up plot ---
fig, ax = plt.subplots(figsize=(8, 5))
pcm = ax.pcolormesh(X, Y, h_series[0], shading='auto', cmap='Blues')
fig.colorbar(pcm, ax=ax, label='Water depth (m)')
ax.set_xlabel("x")
ax.set_ylabel("y")
...
# --- Animation function ---
def update(frame):
    pcm.set_array(h_series[frame].ravel())
    ax.set_title(f"Time = {times[frame]} s")
    return pcm,

# --- Create and save animation ---
ani = animation.FuncAnimation(
    fig, update, frames=len(h_series), interval=30, blit=True
)

ani.save("results/SWE_2D.mp4", fps=20)
plt.show()
