module numerics
    ...
    function boundary_condition(var, var_name) result(var_ext)
        implicit none
        real, dimension(nl, nx, ny) :: var
        real, dimension(nl, 0:nx+1, 0:ny+1) :: var_ext
    ...
    subroutine finite_volume(h, u, v, dh, dhu, dhv)
        implicit none
        real, intent(in), dimension(nl, nx, ny) :: h, u, v
        real, intent(out), dimension(nl, nx, ny) :: dh, dhu, dhv
        real, dimension(nl, 0:nx+1, 0:ny+1) :: h_ext, u_ext, v_ext, hu, hv, huv
        real, dimension(0:nx+1, 0:ny+1) :: flux_x1, flux_x2, flux_y1, flux_y2
        ...
        flux_x1 = hu(1, :, :) * u_ext(1, :, :) + 0.5 * g * h_ext(1, :, :) ** 2
        flux_y1 = hv(1, :, :) * v_ext(1, :, :) + 0.5 * g * h_ext(1, :, :) ** 2
        flux_x2 = hu(2, :, :) * u_ext(2, :, :) + 0.5 * g * h_ext(2, :, :) ** 2 + g_ * h_ext(1, :, :) * h_ext(2, :, :)
        flux_y2 = hv(2, :, :) * v_ext(2, :, :) + 0.5 * g * h_ext(2, :, :) ** 2 + g_ * h_ext(1, :, :) * h_ext(2, :, :)

        dh(1, :, :) = -interface_flux(hu(1, :, :), "zonal") - interface_flux(hv(1, :, :), "meridional")
        dh(2, :, :) = -interface_flux(hu(2, :, :), "zonal") - interface_flux(hv(2, :, :), "meridional")
        dhu(1, :, :) = -interface_flux(flux_x1, "zonal") - interface_flux(huv(1, :, :), "meridional")
        dhu(2, :, :) = -interface_flux(flux_x2, "zonal") - interface_flux(huv(2, :, :), "meridional")
        dhv(1, :, :) = - interface_flux(huv(1, :, :), "zonal") - interface_flux(flux_y1, "meridional")
        dhv(2, :, :) = - interface_flux(huv(2, :, :), "zonal") - interface_flux(flux_y2, "meridional")
    end subroutine finite_volume

    subroutine RK4(h_pre, u_pre, v_pre, h_next, u_next, v_next)
        implicit none
        real, intent(in),  dimension(nl, nx, ny) :: h_pre, u_pre, v_pre
        real, intent(out), dimension(nl, nx, ny) :: h_next, u_next, v_next
        real, dimension(nl, nx, ny) :: k1h, k1hu, k1hv, k2h, k2hu, k2hv, k3h, k3hu, k3hv, k4h, k4hu, k4hv
        real, dimension(nl, nx, ny) :: hu, hv, h_temp, hu_temp, hv_temp, u_temp, v_temp, hu_next, hv_next
